...
#  --output_predictions "../data/anndata/SCPCL000488_muscle.tsv" \
#  --reference "../references/cell_marker_muscle_mtx.csv"
```
...
panglao_binary_mtx <- panglao_connective_tissue |>
  ...
  tidyr::pivot_wider('official gene symbol',
                     ...
                     values_fn = list('cell type' = length),
                     values_fill = list('cell type' = 0)) |> 
...
top_markers_df <- as.list(processed_sce_markers) |>
  purrr::map(\(markers) head(rownames(markers), n = 10)) |>
  dplyr::bind_rows() |>
