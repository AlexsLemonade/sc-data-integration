...
sctype_scores <- function(annotated_sce,
                          model){
...
```{r}
sctype_per_cell <- function(celltype_mtx){
  
  per_cell_assignment <- celltype_mtx |>
    t() |>
    as.data.frame() |>
    tibble::rownames_to_column("barcode") |>
    tidyr::pivot_longer(cols = !"barcode",
                        names_to = "celltype",
                        values_to = "score") |>
    dplyr::group_by(barcode) |>
    dplyr::top_n(n = 1, wt = score) |>
    dplyr::mutate(celltype = ifelse(score < 0.25, 'NA', celltype))
  
  return(per_cell_assignment)
}
```


```{r}
sctype_per_cluster <- function(annotated_sce,
                               celltype_mtx){
  ...
  per_cluster_assignments <- coldata_df |>
  ...
  return(per_cluster_assignments)
...
sctype_results <- purrr::map(model_list, 
                             \(model) sctype_scores(model = model$label.main, 
                                                    annotated_sce = annotated_sce))

# label based on per cell max score
per_cell_results <- purrr::map(sctype_results, sctype_per_cell)

# label based on max cluster score
per_cluster_results <- purrr::map(sctype_results, 
                                  \(result) sctype_per_cluster(annotated_sce = annotated_sce, 
                                                               celltype_mtx = result))
```

```{r}
sctype_results |>
  purrr::iwalk(\(scores, title){
...
```{r}
# print out the table of celltype results on a per cell level
per_cell_results |>
  purrr::map(\(result){
...
```{r}
# print out the table of celltype results on a per cluster level
per_cluster_results |>
  purrr::map(\(result){
    table(result$celltype,
          useNA = "ifany")
...
Looks like we do actually see some more `NA` when we assign by cluster over assigning on a per cell level? 
However, it still doesn't completely fail... 
Looking at the heatmap we also see closer scores between the Adipocytes and Fibroblasts in the no immune reference vs. the HSC scores for all cells. 
I would think if we applied a similar delta metric looking at the scores between the highest score and the median we might see a difference between these two references along with the higher percentage of `NA` assignments.
...
```sh
# activate scvi environment
conda activate scvi

# create cell type assignments
python ../scripts/cell-assign.py \
  --input_anndata "../data/anndata/SCPCL000295_anndata.h5" \
  --output_predictions "../data/anndata/no_bcells_predictions.tsv" \
  --reference "../references/FL_celltype_ensembl.csv" 
  
python ../scripts/cell-assign.py \
  --input_anndata "../data/anndata/SCPCL000295_anndata.h5" \
  --output_predictions "../data/anndata/all_cells_predictions.tsv" \
  --reference "../references/FL_celltype_ensembl-noBcells.csv" 
```
...
```{r}
print_celltype_table <- function(celltype_predictions){
...
print_celltype_table(all_cells_preds)
```


```{r}
# summary of assignment with no B cells 
print_celltype_table(no_bcells_preds)
```
