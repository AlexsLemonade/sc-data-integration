...
- `custom` refers to a reference gene set that contains all cell types found in muscle and connective tissue and only Monocytes from the immune system.
...
**Note:** Running this notebook requires you to download the `<library_id>_processed_celltype.rds` file for the library that you would like to process from `s3://sc-data-integration/scpca/celltype_sce`.
This file should be saved in `data/<sample_id>`. 
...
panglao_tissue_immune_combined <- panglao_df |> 
  dplyr::filter(organ %in% c("Connective tissue", "Smooth muscle", "Skeletal muscle", "Immune system"))
...
all_marker_genes <- list(panglao_tissue_immune_combined,
                         custom_marker_genes)
names(all_marker_genes) <- c("combined", "custom")
...
names(prediction_files) <- c("combined", "custom", "muscle_only")
...
processed_sce$custom_assignments <- all_assignments$custom$celltype
...
coldata_df <- colData(processed_sce) |>
  as.data.frame() |> 
  dplyr::select(barcode, celltype, combined_assignments, custom_assignments, muscle_only_assignments) |>
...
```{r, fig.width=7}
compare_refs_heatmap(original_assignment = coldata_df$custom_assignments,
                     cell_assign = coldata_df$muscle_only_assignments,
                     title = "Custom vs. Muscle only")
```
...
compare_refs_heatmap(original_assignment = coldata_df$custom_assignments,
                     cell_assign = coldata_df$broad_celltype,
                     title = "Custom vs. Submitter annotations")
```
...
It just made it take longer and then assigned cells to immune cells that were not previously identified in our dataset.
