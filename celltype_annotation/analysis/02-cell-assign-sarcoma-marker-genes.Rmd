...
fs::dir_create(cellassign_outs_dir)
...
processed_sce <- readr::read_rds(processed_sce_path)
```
...
Below we will find the upregulated marker genes when comparing each cell type to all other cell types. 
The cell types here were obtained from the submitter.
The original submitter cell types contained sub groups of tumor cells, e.g., `Tumor_myoblast-A` and `Tumor_myoblast-B`. 
Before finding markers, the subgroups are combined into a single cell type, e.g., `Tumor_myoblast`.
These cell types are then used to group cells when applying `scran::FindMarkers()`.
Then the markers are sorted by logFC and the top 10 are selected.
This ensures that we only get marker genes that are upregulated in a specified cell type compared to all other cells in the dataset.
Because of this, we are only identifying marker genes that are unique to each cell type and are not considering marker genes that may be expressed in a subset of cell types.
...
Below the top 10 upregulated markers for each cell type are converted into a binary gene by cell matrix to use as input for `CellAssign`.
...
Perhaps these genes are expressed in other cell types, such as normal myoblast cells that may be present. 
If they are expressed at a high enough level in other cell types then this strategy of "all" used here for `FindMarkers` would not identify those genes. 
Running the same test using "any" however, did not reveal these genes either, or at least they were not in the top 10 (data not shown). 
Another approach could be to filter by a logFC cutoff rather than just taking the top 10.
