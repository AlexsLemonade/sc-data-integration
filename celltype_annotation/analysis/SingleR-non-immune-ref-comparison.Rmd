...
For the rest of this notebook, we will focus on comparing cell type assignments to a complementary reference containing no relevant cells for our sample.
...
ggplot(all_results, aes(x = reference, y = delta.next, group = reference, color = pruned)) +
...
scores_results <- all_results |>
  tidyr::pivot_longer(cols = starts_with("scores"),
                      ...
                      values_to = "score")
ggplot(scores_results, aes(x = reference, y = score, group = reference, color = pruned)) +
  ggforce::geom_sina(size = 0.1, alpha = 0.5) +
  stat_summary(
    aes(group = reference),
    color = "black",
...
label_mtx <- table(pruned_label_main_df$reference,
                   pruned_label_main_df$pruned.labels,
                   useNA = "ifany") |> 
...
pheatmap::pheatmap(label_mtx,
                   ...
                   fontsize_col = 8)
```
Now we will explicitly cmopare our two references that we have been looking at to see how they annotated each of the cells.
