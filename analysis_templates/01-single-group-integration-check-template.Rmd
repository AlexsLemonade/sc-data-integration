...
if(params$celltype_column %in% coldata_names){
  ...
  # Prepare colors for celltypes as a stand-alone variable, based on the first
...
# If there are celltypes, make barplots
if (celltypes_present == TRUE) {
  ...
  plot_barplot_batch_celltype(sce_coldata, params$batch_column, "celltype_plot_names", 
                              ...
                              plot_title = "Top 5 cell types and other") %>% print()
  ...
  plot_barplot_batch_celltype(sce_coldata, 
                              ...
                              plot_title = "All cell types") %>% print()
}
...
extract_labels <- function(colname) {
  as.data.frame(table(sce_coldata[,colname])) %>%
    dplyr::mutate(label = glue::glue("{Var1} (N={Freq})")) %>%
    dplyr::pull(label)
}

# Create batch label vector
batch_plot_labels <- extract_labels(params$batch_column)

# Create celltype label vector, considering top 5 & 'other' (not all celltypes) 
if (celltypes_present) {
  celltype_plot_labels <- extract_labels("celltype_plot_names")
}
```
