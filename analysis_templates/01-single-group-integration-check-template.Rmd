...
The below plot looks at the batch adjusted rand index (ARI) for each integration method. 
Cells are first downsampled to 80% of the total number of cells before clustering is performed using k-means clustering. 
The ARI between clustering results and batch assignments is then calculated and then this is repeated 20 times. 
We also calculated batch ARI across a range of k for k-means, the number of centers used for clustering. 
The plots show the batch ARI on the y-axis across each integration method indicated on the y-axis and are facted by the value of k used for k-means clustering.
