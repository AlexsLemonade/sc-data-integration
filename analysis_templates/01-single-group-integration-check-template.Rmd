...
# Ensure logical
asw_separate_celltypes <- as.logical(toupper(params$asw_separate_celltypes))
if (!(asw_separate_celltypes) %in% c(TRUE, FALSE)) {
  stop("\nCould not read the asw_separate_celltypes` parameter.")
}
```
...
if(params$celltype_column %in% coldata_names){
  ...
  # Prepare celltypes for inclusion in plots, using the unintegrated SCE
  celltype_setup <- setup_plot_celltypes(sce_list$unintegrated)
  
  # Define colors as standalone variable:
  celltype_colors <- celltype_setup$celltype_colors
  
  
  # Add `celltype_names` into all SCEs
  # helper function:
  add_celltype_names <- function(sce, celltype_plot_names) {
    sce$celltype_plot_names <- celltype_plot_names
    return(sce)
  }
  sce_list <- purrr::map(sce_list, add_celltype_names, celltype_setup$celltype_plot_names)
...
if (celltypes_present) {
  # Not separated
  asw_results <- run_asw(sce_list, params$celltype_column, params$seed, by_batch = FALSE) # Provide ALL celltypes here
  print(asw_results$asw_plot)
  
  asw_results <- run_asw(sce_list, "celltype_plot_names", params$seed, by_batch = TRUE, batch_label = "Cell type") # Provide only the subtypes to plot here
  print(asw_results$asw_plot)
} else {
