...
if (celltypes_present == TRUE) {
  ...
  # Create vector of celltype labels with counts for plotting use later
  celltype_plot_labels <- tibble::as_tibble(table(known_celltypes)) %>%
    dplyr::rename(
      celltype_plot_names = known_celltypes
    ) %>%
    dplyr::mutate(label = glue::glue("{celltype_plot_names} (N={n})")) %>%
    dplyr::pull(label)
