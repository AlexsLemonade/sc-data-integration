---
params:
  group_name: "MultipleSclerosisLineageDiversity"
...
source(file.path(utils_dir, "calculate-batch-silhouette-width.R"))
...
```{r}
# Calculate the percentage of cells with cell type information for reporting
percent_present <- round( sum(!(is.na(merged_sce$celltype))) / length(merged_sce$celltype), 3) * 100
```


**Caution:** Only `r percent_present`% of cells in these libraries have cell type annotations. 
...
ilisi_unintegrated <- calculate_lisi(merged_sce,
                                     params$batch_column,
                                     unintegrated=TRUE)
...
plot_lisi(ilisi_df, lisi_type = "iLISI")
```
...
The range of cLISI scores will be from 1 to the number of cell types, however we have normalized the score following the same method utilized by the `scIB` benchmarking tools: (clisi - 1)/(num cell types -1)
...
```{r}
# calculate clisi for unintegrated and integrated SCE separately
clisi_unintegrated <- calculate_lisi(merged_sce,
                                     batch_column = params$celltype,
                                     unintegrated=TRUE)

clisi_integrated_list <- integration_methods %>%
  purrr::imap(~ calculate_lisi(integrated_sce = sce_list[[.x]],
                                batch_column = params$celltype, 
                                integration_method = .x))

# create combined DF 
clisi_df <- dplyr::bind_rows(clisi_unintegrated, clisi_integrated_list)
```


```{r fig.height=5, fig.width=5}
plot_lisi(clisi_df, lisi_type = "cLISI")
```
...
batch_silhouette_unintegrated <- calculate_batch_silhouette_width(merged_sce,
                                                                  seed = params$seed,
                                                                  num_pcs = params$num_pcs,
                                                                  unintegrated=TRUE)

batch_silhouette_integrated_list <- integration_methods %>%
  purrr::imap(~ calculate_batch_silhouette_width(integrated_sce = sce_list[[.x]],
                                                 seed = params$seed,
                                                 num_pcs = params$num_pcs,
                                                 integration_method = .x))

# create combined DF
batch_silhouette_df <- dplyr::bind_rows(batch_silhouette_unintegrated, batch_silhouette_integrated_list)
```

```{r}
plot_batch_asw(batch_silhouette_df, seed = params$seed)
```
