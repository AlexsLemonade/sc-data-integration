...
Cells without annotations are not considered in calculations or visualizations that assess cell type mixing.
  ...
  glue::glue(
      "No cell type information found for this dataset. Cannot make UMAPs annotated with cell type or evaluate cell type mixing."
  )
}
...
if(celltypes_present){
 # create umap plots labeling by celltype column
  ...
  glue::glue("No cell types present so no cell types UMAP will be shown.")
}
```
...
asw_results <- run_asw(sce_list, params$batch_column, params$seed, by_group = TRUE, group_label = "Batch")
...
Calculations follow the same procedure as for batch ASW, except they evaluate the cell type rather than the batch grouping.
...
Here results are not separated by cell type:

```{r fig.height=5, fig.width=5}
aws_results <- run_asw(sce_list, params$celltype_column, params$seed, by_group = FALSE)

# Plot results:
print(aws_results$asw_plot)
```



Here results _are_ separated by cell type:

```{r fig.height=5, fig.width=7}
aws_results <- run_asw(sce_list, params$celltype_column, params$seed, by_group = TRUE, group_label = "Cell type")
