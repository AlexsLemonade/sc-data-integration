...
if (celltypes_present == TRUE) {
  ...
  print(
    glue::glue("Not including NA cell types, there are {nrow(n_celltypes_df)} total cell types, with the following number of cells:")
  )
...
if (celltypes_present == TRUE) {
  plot_barplot_batch_celltype(sce_coldata, params$batch_column, "celltype_plot_names", 
                              ...
                              plot_title = glue::glue("Cell types"))
}
```
...
if (celltypes_present) {
  celltype_plot_labels <- extract_labels(n_celltypes_df, params$celltype_column)
}
```
